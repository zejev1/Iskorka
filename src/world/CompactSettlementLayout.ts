import { isIskorka } from './WorldProfile';
...
function spreadFoundingHumanSettlements(
  ...
  if (world.terrain) return false;
  // Iskorka retains the F2 first-town placement even without its other two towns.
  if (!isIskorka(world) && !FOUNDING_HUMAN_SETTLEMENTS.every((id) => world.settlements[id])) return false;
