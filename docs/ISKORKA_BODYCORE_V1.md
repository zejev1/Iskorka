...
## 16. Реализованный слой конкретных телесных действий

Статус: реализовано в `src/iskorka/BodyActionsV1.ts` и подключено к `WorldEngine`.

### Конкретные действия тела
- еда теперь изменяет `stomachFill`, `energyReserve`, `bowelLoad` и фиксируется как последняя телесная операция `eat`;
- питьё отдельно от еды изменяет `hydration`, водно-солевое отклонение, `bladderFill` и фиксируется как `drink`;
- простое наличие запасов больше не гидратирует тело автоматически;
- мочеиспускание и дефекация происходят как отдельные короткие телесные акты при высоком физиологическом позыве и сбрасывают соответствующее заполнение;
- тело хранит только последнюю телесную операцию, а не бесконечный журнал бытовой физиологии.

### Вода
Питьё возможно при фактическом доступе к одному из источников:
- пресная вода рядом (`river` / `lake`);
- доступ к собственным поселковым/домашним запасам;
- переносимые личные припасы.

Переносимые припасы при питье расходуются. Река/озеро и домашняя вода не создают отдельный тяжёлый объект жидкости на каждую кружку.

### Движение
Фактически потраченное на наземное перемещение мировое время теперь напрямую создаёт:
- `exertionDebt`;
- `muscleFatigue`;
- `oxygenDebt`;
...
Это подключено к уже существующему непрерывному физическому перемещению, поэтому не добавляет новый цикл шагов/сердцебиений.

### Локальная боль
Раны уже имели область тела; теперь из них выводится региональная боль:
- head;
- torso;
- arm;
- leg.

Травма ноги реально увеличивает цену движения. Региональная боль вычисляется из существующих ран и не хранится ещё одной копией в save.

### Прикосновения и объятия
Телесная реакция на контакт рассчитывается отдельно от отношений:
- `touch`;
- `hug`;
...
Контекст зависит от доверия, affinity, respect, conflict, страха, текущей боли и общего телесного дискомфорта. Телесный контакт не пишет любовь/доверие/ценности и не является автоматическим согласием.

Успешный `bond` может дать телесную реакцию объятия; очень тёплое обычное социальное взаимодействие может дать лёгкое добровольное прикосновение без дополнительного случайного броска.

### Проверка
Текущая ветка проходит:
- 63/63 теста;
- standalone tree check до и после сборки;
- TypeScript;
- production build;
